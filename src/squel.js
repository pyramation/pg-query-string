...
function _buildSquel(flavour = null) {
  ...
  cls.Case = class extends cls.BaseBuilder {
    ...
    _toParamString(options = {}) {
      ...
      for (const { expression, values, result } of this._cases) {
        ...
        totalStr += `WHEN ${ret.text} THEN ${this._formatValueForQueryString(
          result
        )}`;
    ...
    _toParamString(options = {}) {
      ...
      if (!totalStr.length) {
        ...
        const fromTableBlock =
          queryBuilder && queryBuilder.getBlock(cls.FromTableBlock);
  ...
  cls.InsertFieldsFromQueryBlock = class extends cls.Block {
    ...
    _toParamString(options = {}) {
      ...
      if (this._fields.length && this._query) {
        ...
        totalStr = `(${this._fields.join(', ')}) ${this._applyNestingFormatting(
          text
        )}`;
        totalValues = values;
      }
  ...
  cls.AbstractConditionBlock = class extends cls.Block {
    ...
    _toParamString(options = {}) {
      ...
      for (const { expr, values } of this._conditions) {
        const ret = cls.isSquelBuilder(expr)
          ? expr._toParamString({
              buildParameterized: options.buildParameterized
            })
  ...
  cls.QueryBuilder = class extends cls.BaseBuilder {
    ...
    _toParamString(options = {}) {
      ...
      const flds = this.blocks.map(b => b._fields);
      const vls = this.blocks.map(b => b._values);
      const opts = this.blocks.map(b => b._valueOptions);

      const coercions = flds.reduce((m, v, idx) => {
        if (!v) return m;
        if (!v.length) return m;
        if (typeof v[0] !== 'string') return m;

        const m2 = v.map((field, i) => ({
          field,
          type: opts[idx][0][i]?.typeCoercion
        }));
      ...
      if (!options.nested) {
        if (options.numberedParameters) {
          ...
          const tc = n => {
            if (coercions[n] && coercions[n].type) {
              return `::${coercions[n].type}`;
            }
            return '';
          };
          totalStr = totalStr.replace(new RegExp(regex, 'g'), function() {
            return `${options.numberedParametersPrefix}${i++}${tc(i - 2)}`;
          });
        }
      }
    ...
    // Deep clone
    clone() {
      const blockClones = this.blocks.map(v => {
        return v.clone();
      });
  ...
  cls.PostgresOnConflictKeyUpdateBlock = class extends cls.AbstractSetFieldBlock {
    ...
    _toParamString(options = {}) {
      ...
      if (this._onConflict) {
        ...
        const conflictFields = this._dupFields
